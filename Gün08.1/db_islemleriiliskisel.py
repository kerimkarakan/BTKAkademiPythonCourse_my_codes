from orm_dbiliskisel import *

Base.metadata.bind = engine  # bind bağlamaya yarıyormuş
...
def kGirisi(k_adi, k_sifre):
    kullanici = (session.query(User).
                 filter(User.user_adi==k_adi,
                                User.user_sifre == k_sifre).first())  #first dedğimiz zaman gelenlerden ilki gelsin diyoruz eğer sorguladığımız şeyden birden çok varsa veritabanında
    return kullanici
